...
function adicionarExercicio() {
    ...
    if (!nomeAluno || !dataTroca || !grupoMuscularKey || !exercicio || !series || !repeticoes) {
...
function atualizarListaExercicios() {
    ...
    exerciciosAdicionados.forEach((ex, index) => {
        ...
        exercicioItem.innerHTML = `
            ...
            <button class="remove-btn" onclick="removerExercicio(${index})">X</button>
        `;
...
function gerarPDFMake() {
    ...
    try {
        const dadosFicha = {
            aluno_nome: nomeAluno, // Passando o nome do aluno
            data_criacao: new Date().toISOString().split("T")[0],
            exercicios: exerciciosAdicionados // Passando o array de exercícios diretamente
        };
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    exerciciosPorGrupo = {
        peitoral: ["Supino", "Crucifixo com halteres"],
        dorsais: ["Barra Fixa", "Remada Curvada com Barra"],
        ombros: ["Desenvolvimento com Halteres", "Elevação Lateral"],
        biceps: ["Rosca Direta com Barra", "Rosca Martelo"],
        triceps: ["Tríceps Testa com Barra", "Tríceps na Polia com Corda"],
        quadriceps: ["Agachamento Livre", "Leg Press 45"],
        posteriores_de_coxa: ["Stiff com Barra", "Mesa Flexora"],
        gluteos: ["Elevação Pélvica", "Cadeira Abdutora"],
        panturrilhas: ["Panturrilha em Pé", "Panturrilha Sentado"],
        trapezio: ["Encolhimento com Barra", "Remada Alta"],
        eretores_da_espinha: ["Hiperextensão Lombar", "Bom Dia"],
        cardio_academia: ["Esteira", "Bicicleta"],
        abdomem: [
            "Abdominal Supra",
            "Prancha Abdominal",
            "Abdominal Crunch",
        ...
        antebracos: ["Rosca de Punho", "Hand Grip"]
    };
    ...
    document.getElementById('data-troca').valueAsDate = new Date();

    // Adicionar event listener para o botão PDF
    const btnPdf = document.getElementById('btn-gerar-pdf');
    if (btnPdf) {
        btnPdf.addEventListener('click', gerarPDFMake);
    }
});
